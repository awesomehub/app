  ...
  switch (action.type) {
    case RecordsetActions.CREATE:
      if (!state.some((recordset) => recordset.id === action.payload.id)) {
        return [...state, recordsetReducer(undefined, action)]
      }
      return state

    case RecordsetActions.RESET:
      return state.map((recordset) =>
        recordset.id === action.payload.id ? recordsetReducer(undefined, action) : recordset,
      )

    case RecordsetActions.DESTROY:
      return state.filter((recordset) => recordset.id !== action.payload.id)
    ...
    case RecordsetActions.SET_PAGE_SIZE:
      return state.map((recordset) =>
        recordset.id === action.payload.id ? recordsetReducer(recordset, action) : recordset,
      )
